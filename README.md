...
Fetches the transcript — pulls captions directly from YouTube, or falls back to transcribing the audio with OpenAI Whisper if captions are unavailable
Summarizes the content — extracts the most important sentences using the LSA algorithm (no model downloads, no API key)
Analyzes sentiment — scores every segment of the transcript from -1.0 (negative) to +1.0 (positive) using VADER, and shows how tone shifts across the video
Saves results — exports everything to a JSON file for further use
...
Download from python.org. During installation, tick "Add Python to PATH".
2. Install dependencies
powershellpython -m pip install youtube-transcript-api yt-dlp sumy nltk requests

If python is not recognized, use the full path:
powershellC:\Users\<you>\AppData\Local\Programs\Python\Python313\python.exe -m pip install youtube-transcript-api yt-dlp sumy nltk requests

3. Set up cookies.txt (required)
YouTube rate-limits unauthenticated transcript requests. To bypass this:

Install the "Get cookies.txt LOCALLY" extension in Chrome
Log into youtube.com
Click the extension → Export → save as cookies.txt
Place cookies.txt in the Summ/ folder


Usage
Open main.py and change the URL at the bottom:
pythonTEST_URL = "https://www.youtube.com/watch?v=YOUR_VIDEO_ID"
Then run:
...
Dependencies
PackagePurposeyoutube-transcript-apiFetch YouTube captionsyt-dlpDownload audio for Whisper fallbackopenai-whisperTranscribe audio locallysumyExtractive summarizationnltkSentence tokenization + VADER sentimentrequestsPass cookies to bypass rate limiting
